...
interface EditGooglePostFormProps {
    post: { id: string } & GooglePostFormType;
    locationId: string;
    accountId: string;
    onSuccess?: () => void;
}

export const EditGooglePostForm: React.FC<EditGooglePostFormProps> = ({
    post,
    locationId,
    accountId,
    onSuccess,
}) => {
    const { updatePost, loading, successMessage, error, clearMessages } =
        useGooglePosts();
    ...
    return (
        <Card className="w-full bg-neutral-900 border border-white/10">
            <CardContent className="p-6 space-y-6">
                <h2 className="text-xl font-bold text-white">Edit Google Post</h2>
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <Select
                            defaultValue={post.postType || "UPDATE"}
                    ...
                    <Button
                        type="submit"
                        disabled={loading}
                        className="w-full bg-gradient-to-r from-yellow-400 to-red-500 text-white font-semibold"
                    >
                        Update Post
                    </Button>
                </form>
